...
class FileHandler(RequestHandler):
    def post(self):
        ...
        print(param)
        ret, res = self.application.handler_file_upload(param)
        self.write(json.dumps({"ret": ret, "msg": res}))
...
class RunHandler(RequestHandler):
    def post(self):
        ...
        param = json.loads(self.request.body.decode())
        print(param)
        ret, res = self.application.handler_run_script(param)
        self.write(json.dumps({"ret": ret, "msg": res}))
...
class StopHandler(RequestHandler):
    def post(self):
        print("stop script")
        ret, res = self.application.handler_stop_script({})
        self.write(json.dumps({"ret": ret, "msg": res}))


class Slave(Application):
    def __init__(self):
        ...
        super(Slave, self).__init__(handlers, **settings)
